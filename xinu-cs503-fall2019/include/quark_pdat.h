...
/* Structures and Values used to read the platform data from the memory mapped
 * SPI flash */
#define QUARK_PDAT_MAGIC "PDAT" /* Platform header signature */
#define QUARK_PDAT_MEMORY_LOC \
  0xFFF10000 /* Memory mapped location of the platform data */
...
struct quark_pdat_header {
  ...
  uint32 quark_pdat_len;    /* Size in bytes of the platform data (not including
                               the header */
  uint32 quark_pdat_crc; /* CRC value for the platform data (not including the
                            header) */
...
struct quark_pdat_entry {
  ...
  uint16 pdat_entry_len; /* Length in bytes of the platform entry (not including
                            the entry header) */
...
int get_quark_pdat_entry_data_by_desc(const char* entry_desc, char* buf,
                                      uint32 size);
