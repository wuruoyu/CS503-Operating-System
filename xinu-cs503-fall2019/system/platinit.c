...
#define QUARK_CONS_BAR_INDEX \
  0 /* Index: console's MMIO \
       base addr register */
#define QUARK_CPUID_VALUE        \
  0x590 /* CPUID value for Intel \
           Quark microprocessor */
#define CPUID_MAJOR_MASK           \
  0xfffffff0 /* Mask for CPUID w/o \
                cpu stepping value */
...
int console_init(void) {
  ...
  pciDev = find_pci_device(INTEL_QUARK_UART_PCI_DID, INTEL_QUARK_UART_PCI_VID,
                           QUARK_CONS_PORT);
  if (pciDev < 0) {
...
void platinit() {
  ...
  pcidev = find_pci_device(INTEL_ETH_QUARK_PCI_DID, INTEL_ETH_QUARK_PCI_VID, 0);
  if (pcidev != SYSERR) {
    /* Store the pcidev for future use */
    ethertab[0].pcidev = pcidev;

    pci_get_dev_mmio_base_addr(pcidev, 0, &devtab[ETHER0].dvcsr);
  }
}
