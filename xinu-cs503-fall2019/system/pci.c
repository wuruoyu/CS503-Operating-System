...
static inline void *regAddress_MMConfig(
  ...
  retval = (void *)(mmconfigBase + (bus * PCIE_BYTES_PER_BUS) +
                    (device * PCIE_BYTES_PER_DEVICE) +
                    (function * PCIE_BYTES_PER_FUNCTION) + offset);
  return retval;
}
 ...
 *------------------------------------------------------------------------
 */
static inline uint32 encodePCIDevice(uint32 bus, /* PCI bus number	*/
 ...
 *------------------------------------------------------------------------
 */
static inline uint32 encodedPCIDevToDevice(uint32 encodedDev) {
 ...
 *------------------------------------------------------------------------
 */
int32 find_pci_device(int32 deviceID, /* PCI device ID	*/
  ...
  for (bus = 0; bus < PCI_MAX_BUSES; bus++) {
    for (dev = 0; dev < PCI_DEVICES_PER_BUS; dev++) {
      for (func = 0; func < PCI_FUNCTIONS_PER_DEVICE; func++) {
        devfuncHdr = regAddress_MMConfig(MMCONFIG_BASE, bus, dev, func, 0);
        if ((devfuncHdr->vendorID == vendorID) &&
            (devfuncHdr->deviceID == deviceID) && (index == count++)) {
        ...
        if (func == 0) {
          multifunction = devfuncHdr->headerType & PCI_HDR_TYPE_MULTIFCN;
          if (!multifunction) {
            break; /* Inner for */
          }
        }
...
int32 pci_get_dev_mmio_base_addr(
  ...
  status = pci_read_config_dword(
      encodedDev, offsetof(struct pci_config_header, bars[barIndex]),
      &bar_value);
  if (status < 0) {
    return status;
  }
 ...
 *------------------------------------------------------------------------
 */
int32 pci_set_ivec(uint32 pcidev, /* Encoded PCI device	*/
