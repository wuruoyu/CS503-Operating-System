/* quark_pdat.c - Functions for retrieving platform data from the quark SPI
 * flash */
...
int get_quark_pdat_entry_data_by_id(uint16 type_id, char* buf, uint32 size) {
  uint32 offset = 0;
  struct quark_pdat_header* header =
      (struct quark_pdat_header*)QUARK_PDAT_MEMORY_LOC;
  ...
  while (offset < header->quark_pdat_len) {
    ...
    entry = (struct quark_pdat_entry*)(((char*)entry) +
                                       (sizeof(struct quark_pdat_entry) +
                                        entry->pdat_entry_len));
